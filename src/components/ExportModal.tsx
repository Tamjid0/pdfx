...
export const ExportModal: React.FC<ExportModalProps> = () => {
    ...
    return (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-md bg-[#111] border border-[#333] rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
                <div className="p-6 border-b border-[#222] bg-[#1a1a1a] flex items-center justify-between">
                    <div>
                        <h2 className="text-xl font-bold text-white">Export Options</h2>
                        <p className="text-xs text-gray-400 mt-1">Select your preferred format for {mode}</p>
                    </div>
                    <button
                ...
                <div className="p-6 space-y-4">
                    {getOptions().map((opt) => (
                        <button
                            ...
                            disabled={isExporting}
                            className={`w-full group flex items-center gap-4 p-4 rounded-2xl border transition-all duration-300 ${isExporting ? 'opacity-50 cursor-not-allowed' : 'bg-[#1a1a1a] border-[#333] hover:border-[#00ff88] hover:bg-[#1a1a1a]/50'}`}
                        >
                            <div className="w-12 h-12 flex items-center justify-center text-2xl bg-[#222] rounded-xl group-hover:bg-[#00ff88]/10 transition-colors">
                                {opt.icon}
                            </div>
                            <div className="text-left flex-1">
                                <div className="text-sm font-bold text-white group-hover:text-[#00ff88] transition-colors">{opt.label}</div>
