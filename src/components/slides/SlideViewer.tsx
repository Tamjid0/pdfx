...
const SlideViewer: React.FC = () => {
    ...
    // Check if we should use the PDF viewer (Production Grade PPTX Rendering)
    // We assume if it's in slide mode, it might have a converted PDF.
    ...
    return (
        <div className="slide-viewer flex flex-col h-full bg-[#0a0a0a] relative group overflow-hidden animate-in fade-in duration-500">
            ...
            <div className="flex-1 flex items-center justify-center p-4 relative">
                {isSlideMode && pdfUrl ? (
                    // HIGH FIDELITY PDF VIEWING (LibreOffice Result)
                    <div className="w-full h-full flex flex-col items-center">
                        <div className="slide-content-container w-full h-full bg-[#1a1a1a] rounded-xl border border-[#333] shadow-2xl relative overflow-hidden transition-all duration-300 hover:border-[#00ff88]/30">
                            <iframe
                        ...
                        <div className="mt-4 text-[10px] font-bold text-[#00ff88] uppercase tracking-[0.2em]">
                            High-Fidelity Document Preview
                        </div>
                    </div>
                ) : (
                    // TEXT-BASED FALLBACK (Legacy or other formats)
                ...
                {/* Floating Navigation Controls (Only for text-based fallback, PDF viewer has its own or we handle it) */}
                {!pdfUrl && (
                    <>
            ...
            {/* Bottom Progress Bar (Only for text-based slides) */}
