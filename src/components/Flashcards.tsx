...
const Flashcards: React.FC<FlashcardsProps> = ({ onGenerate }) => {
    ...
    const hasData = cardsArray.length > 0;
        ...
        <div className="flex flex-col h-full w-full bg-[#0a0a0a] rounded-xl border border-[#222] overflow-hidden shadow-2xl relative">
            <div className="flex items-center justify-between p-5 border-b border-[#222] bg-[#111]">
                <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-xs font-black text-white uppercase tracking-[0.3em]">{hasData ? 'Knowledge Deck' : 'Deck Calibration'}</h3>
                </div>
                <div className="flex items-center gap-3">
                    {hasData && (
                        <button
                    ...
                    >
                        ...
                        {isGeneratingFlashcards ? 'CREATING...' : (hasData ? 'REGENERATE' : 'SETUP')}
                    </button>
                </div>
            </div>

            <VersionTabs module="flashcards" />
            ...
            <div className="p-6 bg-[#0f0f0f] border-t border-[#222] flex justify-between items-center">
                <button
                    onClick={() => hasData && setFlashcardsData({ ...flashcardsData!, flashcards: [] })}
                    className={`text-[10px] font-black text-[#444] uppercase tracking-widest hover:text-white transition-all ${!hasData ? 'opacity-30 cursor-not-allowed' : ''}`}
                    disabled={!hasData}
                >
                    Clear Session
                </button>
                <div className="flex gap-4">
                    <button
                        ...
                        className={`px-6 py-2.5 bg-[#444] text-white rounded-xl text-xs font-bold hover:bg-[#666] transition-all flex items-center gap-2 ${!hasData ? 'opacity-50 cursor-not-allowed' : ''}`}
                    ...
                    <button
                        onClick={() => hasData && openExportModal('flashcards', flashcardsData)}
                        className={`px-8 py-2.5 bg-[#00ff88] text-black rounded-xl text-xs font-bold hover:bg-[#00dd77] transition-all shadow-xl active:scale-95 ${!hasData ? 'opacity-50 cursor-not-allowed' : ''}`}
                        disabled={!hasData}
                    >
                        EXPORT JSON
                    </button>
