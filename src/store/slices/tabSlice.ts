...
export const generateDefaultDraftId = () => `draft-${Math.random().toString(36).substring(2, 9)}-${Date.now()}`;

export const getInitialTabbingState = () => {
    const modules: Mode[] = ['summary', 'notes', 'insights', 'flashcards', 'quiz', 'mindmap'];
    const localDrafts: any = {};
    const activeRevisionIds: any = {
        summary: '', notes: '', insights: '', flashcards: '', quiz: '', mindmap: '', editor: '', chat: '', slides: ''
    };

    modules.forEach(m => {
        const id = generateDefaultDraftId();
        localDrafts[m] = [{ id, name: 'Draft 1', data: null }];
        activeRevisionIds[m] = id;
    });

    return { localDrafts, activeRevisionIds };
};

export const createTabSlice: StateCreator<AppState, [], [], TabSlice> = (set, get) => {
    const initialState = getInitialTabbingState();

    return {
        localDrafts: initialState.localDrafts,
        summaryRevisions: [],
        notesRevisions: [],
        insightsRevisions: [],
        flashcardsRevisions: [],
        quizRevisions: [],
        mindmapRevisions: [],
        activeRevisionIds: initialState.activeRevisionIds,
